#!/usr/bin/env python3
#
# Author: INDRAKSHI MANDAL (INDRAKSHI) 
#
...
from Core.indu_core import Payload_generator, initiate_hoax_server, Sessions_manager, Hoaxshell, Core_server, Netcat_multi_listener
...
def print_banner():
	...
	print(f"   {''.join(final)}")
	print(f'{END}{padding}           by INDRAKSHI\n')
...
class PrompHelp:
	...
	commands = {
		...
		'connect' : {
			'details' : f''' 			
			\r  Connect with another machine running indu (sibling server). Once connected, you will be able 
		...
		'generate' : {
			'details' : f''' 			
			\r  Generate backdoor payload. If you start indu with SSL the generated payload(s) 
		...
		'repair' : {
			...
			\r  in case indu does not interpret the information correctly when a
		...
		'siblings' : {
			'details' : f'''
			\r  Siblings are basically other instances of indu that you've connected with.
			''',
			'least_args' : 0,
			'max_args' : 0
		},
...
def main():
	...
	if args.update:
		...
		try:
			...
			u = check_output(f'cd {cwd}&&git pull https://github.com/indrakshi2121/indu main', shell=True).decode('utf-8')

			if re.search('Updating', u):
				print(f'[{INFO}] Update completed! Please, restart indu.')
				updated = True
				...
				print(f'[{FAILED}] Something went wrong. Are you running indu from your local git repository?')
				print(f'[{DEBUG}] Consider running "git pull https://github.com/indrakshi2121/indu main" inside the project\'s directory.')

		except:
			print(f'[{FAILED}] Update failed. Consider running "git pull https://github.com/indrakshi2121/indu main" inside the project\'s directory.')
